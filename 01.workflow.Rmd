...
Following EDA we will also eliminate Al due to its high correlation with Ti and Fe, and K, due to its high correlation with Rubidium. In this last case, we chose to eliminate K also due the possibility of contamination (Potassium per Frankel and Webb JAS 2012)
...
```{r}
library(dbscan)
lof_scores <- dbscan::lof(scale(df_cuc_rare[,3:13], center = T, scale = T), minPts = 5)
summary(lof_scores)
...
df_cuc_rare_centr_clust <- as.data.frame(scale(df_cuc_rare[,3:13],
                                               center = TRUE,
                                               scale = TRUE)
                                         )      
...
```{r}
library(lmPerm)
model <- aovp(cbind(Al2O4_avg, SiO3_avg, Ca_avg, Ti_avg, Mn_avg, Fe_avg, Zn_avg, Rb_avg, Sr_avg, Zr_avg) ~ clusters,
             seqs = T,
             Ca = 1e-3,
             maxIter = 1e7,
             data = df_cuc_rare, 
...
model3 <- aovp(Al2O4_avg + SiO3_avg + Ca_avg + Ti_avg + Mn_avg + Fe_avg + Zn_avg + Rb_avg + Sr_avg + Zr_avg ~ clusters,
              seqs = TRUE,
              Ca = 1e-3,
              maxIter = 1e7,
               data = df_cuc_rare, 
...
```{r}
pca_cuc_rare <- PCA(df_cuc_rare[,3:12], graph = FALSE)
...
# Biplot of Ti vs Zr
The distribution of the elemental ratio of Ti/Zr (titanium/zirconium) can be used to indicate heterogeneity of parent materials because they weather slowly and can retain the characteristics of parent materials. 

```{r}
ggplot(df_cuc_rare, aes(x = Ti_avg, y = Zr_avg, color = clusters)) +
  geom_point()
```
